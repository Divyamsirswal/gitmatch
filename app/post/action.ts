...
import { type Card as BaseCard } from "./page";
...
const calculateSkillScore = (level1: string, level2: string): number => {
    ...
    if (level1 === 'INTERMEDIATE' && (level2 === 'BEGINNER' || level2 === 'ADVANCED')) return 8;
    if (level2 === 'INTERMEDIATE' && (level1 === 'BEGINNER' || level1 === 'ADVANCED')) return 8;
    if ((level1 === 'BEGINNER' && level2 === 'ADVANCED') || (level1 === 'ADVANCED' && level2 === 'BEGINNER')) return 2;
    return 0;
};

const parseTimezoneOffset = (tzString?: string): number | null => {
    if (!tzString) return null;

    if (tzString.toUpperCase() === 'IST') return 5.5;

    const match = tzString.toUpperCase().match(/(?:GMT|UTC)\s*([+-])(\d{1,2})(?::?(\d{2}))?/);
    if (match) {
...
const calculateAvailabilityOverlap = (avail1?: string[], avail2?: string[]): number => {
    if (!avail1 || !avail2 || avail1.length === 0 || avail2.length === 0) return 0;
...
export async function findMatches(newCard: NewCardInput): Promise<MatchedCard[]> {
    ...
    const scoredMatches = potentialMatches.map((match): MatchedCard => {
        ...
        if (commonTags.length > 0) {
            ...
            if (commonTags.length === newCard.tech_tags.length && commonTags.length === match.tech_tags.length) {
                score += 5;
            }
        }
        ...
        if (newCardTzOffset !== null) {
            ...
            if (!match.timezone) score += 1;
        }


        score += calculateAvailabilityOverlap(match.availability, newCard.availability);
    ...
    const relevantMatches = scoredMatches
        ...
        .filter(match => !(
            ...
            JSON.stringify(match.tech_tags.sort()) === JSON.stringify(newCard.tech_tags.sort())
        ))
